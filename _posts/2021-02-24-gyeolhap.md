...
코드에서는 분명히 bounds의 insets을 넣고싶은 셀의 수만큼 잘 나눠서 코드를 써줌
```swift
...
- [위의 문제4 해결] 기존의 고정된 width와 height를 가졌던 타일들을 collevtion View의 크기에 따라 자신의 사이즈를 조절하게해서 해결
...
- [문제5 해결] 완벽한 해결은 아니라 찜찜하지만 추후에 더 알아보기로 하고 급한 불을 껐다.
  - navigation bar를 hide시키는 방법과 라지 타이틀을 기본 타이틀로 바꾸는 방법이 있었다.
  - 네비 바를 없애면 뒤로 가기 버튼을 만들어야 한다는 단점이 있었고 타이틀 자리가 아예 사라진다.
  - 기본 타이틀로 바꾸면 디자인은 별로지만 뒤로가기 버튼과 스테이지 타이틀이 살아있다는 장점이 있어서 이 방법을 택했다.
  - 뭔가 [이 사이트](https://www.knowband.com/blog/mobile-app/remove-extra-space-navigation-bar-scroll-view/)의 해결방법으로 해결할 수 있을 줄 알았는데 스토리보드를 쪼개놔서인지 promatically하게 적용해봐도 해결되지 않았다.

<figure>
<img data-action="zoom" src='{{ "/assets/images/2021-02-24/6라지타이틀지워서해결.gif" | relative_url }}' width=250 alt='absolute'>
<figcaption>[문제5의 해결]네비바와 타일 사이의 빈공간은 사라졌지만 디자인이 구식이 됐다.</figcaption>
</figure>

- [문제6]  collectionView.bounds.width로 딱 맞게 계산했는데도 불구하고 삐져나가거나 inset이 안 맞는 문제
<img data-action="zoom" src='{{ "/assets/images/2021-02-24/img1.png" | relative_url }}' width=250 alt='absolute'>
<img data-action="zoom" src='{{ "/assets/images/2021-02-24/img2.png" | relative_url }}' width=250 alt='absolute'>
...
- [문제1] 제 게임에서는 3번째 타일을 모두 클릭하는 순간 선택된 모든 타일이 초기화 되어야 하는데 마지막에 클릭한 타일만 초기화되는 문제가 생겼습니다.
  - [실마리] 이에 대한 해결책으로 collectionView.reloadData라는 메서드를 알아냈지만 reloadDate를 쓰면 된다고 인터넷에서 보았습니다.
...
- [적용] 위 문제는 잠시 제쳐두고 아래의 정답을 적을 컬렉션 뷰를 추가하기로 함 
...
- [문제4] 하지만 기존 타일들의 레이아웃이 짤리는 문제 발생
